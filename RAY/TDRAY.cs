...
public class TDRAY : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        CS.SetVector("_LightDir", transform.position);
        ...
        CS.SetVector("input", new Vector4((Input.GetKey(KeyCode.A)) ? -1 : (Input.GetKey(KeyCode.D) ? 1 : 0), 0, 0, 0));
    ...
    void Buff()
    {
        cb = new ComputeBuffer(1, sizeof(float) * 2);
        cb.SetData(new Vector2[1] { new Vector2(-2, -2) });

        CS.SetBuffer(0, "_Player", cb);
        CS.SetBuffer(1, "_Player", cb);
    }
